...
SCRIPT_ID = "learning-v2-controlled-source-change-git-commit-gate-v0"
...
def main():
    ...
    if git_diff_audit.get("audit_status") != "controlled_source_change_git_diff_audit_ready_for_git_commit_gate_dry_run":
        hard_blocks.append("git_diff_audit_not_ready_for_git_commit_gate")
    if git_diff_audit.get("git_commit_gate_dry_run_allowed") is not True:
        hard_blocks.append("git_commit_gate_dry_run_not_allowed")
    if git_diff_audit.get("git_commit_allowed") is not False:
        hard_blocks.append("git_diff_audit_allows_git_commit_too_early")
    if git_diff_audit.get("git_push_allowed") is not False:
        hard_blocks.append("git_diff_audit_allows_git_push_too_early")
    ...
    if git_diff_audit.get("hard_blocks"):
        hard_blocks.append("git_diff_audit_report_has_hard_blocks")
    ...
    if post_validation.get("git_commit_allowed") is not False:
        hard_blocks.append("post_validation_allows_git_commit_too_early")
    if post_validation.get("git_push_allowed") is not False:
        hard_blocks.append("post_validation_allows_git_push_too_early")
    ...
    candidate_files = git_diff_audit.get("candidate_files") or []
    ...
    if git_status_all["returncode"] != 0:
        hard_blocks.append("git_status_all_failed")
    if git_status_candidates["returncode"] != 0:
        hard_blocks.append("git_status_candidates_failed")
    if git_diff_candidates["returncode"] != 0:
        hard_blocks.append("git_diff_candidates_failed")
    if git_log_head["returncode"] != 0:
        hard_blocks.append("git_log_head_failed")
    ...
    if candidate_status_lines:
        hard_blocks.append("candidate_files_dirty_before_commit_gate")
    if candidate_diff_lines:
        hard_blocks.append("candidate_files_have_diff_before_commit_gate")
    ...
    for rel in candidate_files:
        ...
        commit_gate_items.append({
            ...
            "would_allow_commit_if_real_write_existed": True,
            "git_commit_allowed": False,
            "git_push_allowed": False,
            "cloudflare_deploy_allowed": False,
            "deploy_allowed": False,
            "requires_git_push_gate_before_cloudflare_pages_deploy": True,
    ...
    if hard_blocks:
        gate_status = "blocked"
        recommended_next_action = "fix_git_commit_gate_inputs"
        git_push_gate_dry_run_allowed = False
    else:
        gate_status = "controlled_source_change_git_commit_gate_ready_for_git_push_gate_dry_run"
        recommended_next_action = "run_controlled_source_change_git_push_gate_dry_run"
        git_push_gate_dry_run_allowed = True
    ...
    payload = {
        ...
        "commit_gate_items": commit_gate_items,
        ...
        "git_push_gate_dry_run_allowed": git_push_gate_dry_run_allowed,
        "git_commit_allowed": False,
        "git_push_allowed": False,
    ...
    ts = stamp()
    json_path = REPORT_DIR / f"learning-v2-controlled-source-change-git-commit-gate-dry-run-{ts}.json"
    md_path = SNAPSHOT_DIR / f"learning-v2-controlled-source-change-git-commit-gate-dry-run-{ts}.md"

    json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    md = []
    md.append("# Learning V2 Git Commit Gate Dry Run")
    md.append("")
    ...
    md.append(f"- git_push_gate_dry_run_allowed: `{str(git_push_gate_dry_run_allowed).lower()}`")
    ...
    print("controlled_source_change_git_commit_gate = ok")
    print("mode = dry_run")
    ...
    print("git_push_gate_dry_run_allowed =", str(git_push_gate_dry_run_allowed).lower())
