...
SCRIPT_ID = "learning-v2-controlled-source-change-real-write-request-v0"
...
def main():
    ...
    ap.add_argument("--apply", action="store_true", help="Reserved; v0 never writes website source.")
    args = ap.parse_args()
    ...
    if apply_auditor.get("real_write_request_dry_run_allowed") is not True:
        hard_blocks.append("real_write_request_dry_run_not_allowed")
    if apply_auditor.get("source_change_gate_allowed") is not False:
        hard_blocks.append("apply_auditor_allows_gate_too_early")
    if apply_auditor.get("source_change_gate_opened") is not False:
        hard_blocks.append("apply_auditor_opened_gate")
    if apply_auditor.get("hard_blocks"):
        hard_blocks.append("apply_auditor_has_hard_blocks")
    ...
    if apply_report.get("source_change_gate_allowed") is not False:
        hard_blocks.append("controlled_apply_report_allows_gate_too_early")
    ...
    for item in apply_plan:
        ...
        if item.get("will_write_source") is not False:
            item_blocks.append("apply_dry_run_will_write_source_not_false")
        if item.get("will_commit") is not False:
            item_blocks.append("apply_dry_run_will_commit_not_false")
        if item.get("will_push") is not False:
            item_blocks.append("apply_dry_run_will_push_not_false")
        if item.get("will_deploy") is not False:
            item_blocks.append("apply_dry_run_will_deploy_not_false")

        real_write_request_items.append({
            "path": rel,
            "requested_operation": item.get("operation") or "previewed_change",
            "source_exists": item.get("source_exists"),
            "request_real_source_write": True,
            "write_must_be_separately_audited": True,
    ...
    request_packet = {
        "request_type": "controlled_source_change_real_write_request",
        "requested_stage": "controlled_source_change_real_write",
        "candidate_files": candidate_files,
        "request_items": real_write_request_items,
        "required_next_before_any_write": [
    ...
    if hard_blocks:
        ...
        recommended_next_action = "fix_controlled_real_write_request_inputs"
        request_audit_allowed = False
    else:
        request_status = "controlled_source_change_real_write_request_ready_for_audit"
    ...
    ts = stamp()
    json_path = REPORT_DIR / f"learning-v2-controlled-source-change-real-write-request-dry-run-{ts}.json"
    md_path = SNAPSHOT_DIR / f"learning-v2-controlled-source-change-real-write-request-dry-run-{ts}.md"

    json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    md = []
    md.append("# Learning V2 Controlled Source-Change Real-Write Request Dry Run")
    md.append("")
    ...
    print("controlled_source_change_real_write_request = ok")
    print("mode = dry_run")
