...
def write_report(state, probe_path, probe, stage_after, apply):
    ...
    out = REPORT_DIR / f"accessibility-nav-probe-resolver-{suffix}-{stamp()}.md"
    ...
    lines = []
    lines.append("# Accessibility Nav Probe Resolver")
    lines.append("")
...
def main():
    ...
    print("accessibility_nav_probe_resolver =", "apply" if args.apply else "dry_run")
    ...
    if state.get("current_stage") != "accessibility_nav_probe":
        failures.append(f"current_stage_not_accessibility_nav_probe:{state.get('current_stage')}")
    ...
    if review_count > 0:
        stage_after = "accessibility_nav_review_ready"
        resolver_result = "would_review_ready"
    else:
        stage_after = "accessibility_nav_track_complete"
        resolver_result = "would_track_complete"

    if failures:
        print("accessibility_nav_probe_resolver_result = blocked")
        for x in failures:
            print("failure =", x)
    ...
    print(
        "accessibility_nav_probe_resolver_result =",
        "review_ready" if args.apply and stage_after == "accessibility_nav_review_ready" else resolver_result
    )
    ...
    state["history"].append({
        "at": now_iso(),
        "executor": "accessibility_nav_probe_resolver",
        "stage_before": "accessibility_nav_probe",
    ...
    state["last_accessibility_nav_probe_resolver"] = {
        "at": now_iso(),
        "result": stage_after,
        "target_family": TARGET_FAMILY,
        "probe_report": str(probe_path),
    ...
    state["next_action"] = (
        "Review accessibility navigation semantics evidence and design a proposal-only executor. "
