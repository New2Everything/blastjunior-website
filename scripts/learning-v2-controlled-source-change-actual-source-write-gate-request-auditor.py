...
SCRIPT_ID = "learning-v2-controlled-source-change-actual-source-write-gate-request-auditor-v0"
...
MUST_REMAIN_FALSE = {
    ...
    "actual_source_write_gate_opened",
...
def main():
    ...
    ap.add_argument("--apply", action="store_true", help="Reserved; v0 is report-only and never opens the actual source-write gate.")
    args = ap.parse_args()
    ...
    if gate_request.get("request_status") != "controlled_source_change_actual_source_write_gate_request_ready_for_audit":
        hard_blocks.append("gate_request_not_ready_for_audit")
    if gate_request.get("gate_request_audit_allowed") is not True:
        hard_blocks.append("gate_request_audit_not_allowed")
    ...
    if gate_request.get("deploy_allowed") is not False:
        hard_blocks.append("gate_request_deploy_allowed_too_early")
    if gate_request.get("hard_blocks"):
        hard_blocks.append("gate_request_report_has_hard_blocks")
    ...
    if packet.get("request_type") != "controlled_source_change_actual_source_write_gate_request":
        hard_blocks.append("gate_request_packet_type_invalid")
    if packet.get("requested_stage") != "controlled_source_change_actual_source_write_gate":
        hard_blocks.append("gate_request_requested_stage_invalid")
    ...
    for item in gate_items:
        ...
        if item.get("requires_gate_request_auditor") is not True:
            item_blocks.append("requires_gate_request_auditor_not_true")
    ...
    safety = gate_request.get("safety") or {}
    for k in [
        "business_source_written",
        "website_source_written",
        "git_commit",
    ...
    if hard_blocks:
        ...
        recommended_next_action = "fix_actual_source_write_gate_request_before_gate_opener"
        gate_opener_dry_run_allowed = False
    else:
        audit_status = "controlled_source_change_actual_source_write_gate_request_ready_for_gate_opener_dry_run"
    ...
    ts = stamp()
    json_path = REPORT_DIR / f"learning-v2-controlled-source-change-actual-source-write-gate-request-auditor-dry-run-{ts}.json"
    md_path = SNAPSHOT_DIR / f"learning-v2-controlled-source-change-actual-source-write-gate-request-auditor-dry-run-{ts}.md"

    json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    md = []
    md.append("# Learning V2 Actual Source-Write Gate Request Auditor Dry Run")
    md.append("")
    ...
    print("controlled_source_change_actual_source_write_gate_request_auditor = ok")
    print("mode = dry_run")
