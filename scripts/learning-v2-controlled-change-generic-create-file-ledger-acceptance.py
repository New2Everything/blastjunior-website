...
ACCEPTANCE_ID = "learning-v2-controlled-change-generic-create-file-ledger-acceptance-v0"
...
def validate_validation_result(item):
    failures = []
    warnings = []
    ...
    if item.get("result") != "ok":
        failures.append(f"validation_result_not_ok:{item.get('result')}")

    if item.get("source_written") is not True:
        failures.append(f"validation_source_written_not_true:{item.get('source_written')}")

    if item.get("isolated_delta_interpretation") != "missing_to_current_delta_is_created_target_file_only":
        failures.append(f"isolated_delta_not_clean:{item.get('isolated_delta_interpretation')}")
    ...
    elif item.get("added_line_count") <= 0:
        failures.append(f"added_line_count_not_positive:{item.get('added_line_count')}")
    ...
    if item.get("failures"):
        failures.append("validation_item_has_failures")

    return failures, warnings

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--apply", action="store_true", help="accept validated create-file delta into dirty-freeze")
    args = ap.parse_args()
    ...
    if isolated.get("source_written") is not False:
        failures.append(f"isolated_top_level_source_written_not_false:{isolated.get('source_written')}")
    ...
    if not failures:
        for item in validation_results:
            item_failures, item_warnings = validate_validation_result(item)
            target_file = str(item.get("target_file") or "").lstrip("./")

            if item_warnings:
                warnings.extend([f"{target_file}:{x}" for x in item_warnings])
    ...
    blocked_acceptance_results = [x for x in acceptance_results if x.get("result") != "ok"]

    if blocked_acceptance_results:
        failures.append(f"blocked_create_file_acceptance_results:{len(blocked_acceptance_results)}")

    metadata_file_count = isolated.get("metadata_file_count", 0)

    if metadata_file_count == 0:
        acceptance_mode = "no_metadata_safe_noop"
        recommended_next_step = "close_create_file_lifecycle_extension_summary"
    else:
        acceptance_mode = "create_file_ledger_acceptance"
        recommended_next_step = (
            "close_create_file_lifecycle_extension_summary"
            if not failures
            else "fix_generic_create_file_ledger_acceptance_blockers"
        )
    ...
    if args.apply and result == "ok" and controlled_delta_paths:
        ...
        if result == "ok":
            freeze_path = FREEZE_DIR / f"dirty-freeze-controlled-create-file-{run_stamp}.json"
            new_freeze = {
                "generated_at": now_iso(),
                "freeze_type": "generic_create_file_controlled_business_source_change_acceptance",
                "acceptance_id": ACCEPTANCE_ID,
                "accepted_controlled_delta_paths": actual_delta_paths,
                "reason": "Accept validated generic create-file controlled source change after isolated missing-to-current verification.",
                "previous_dirty_freeze": state.get("last_dirty_freeze"),
                "source_reports": {
                    "new_file_template_validator": str(template_validator_path),
                    "generic_create_file_dry_run_runner": str(dry_runner_path),
                    "generic_create_file_policy_gate": str(policy_gate_path),
                    "generic_create_file_readiness": str(readiness_path),
                    "generic_create_file_apply": str(apply_path),
                    "generic_create_file_isolated_validator": str(isolated_path),
                },
            ...
            state["last_controlled_business_change_acceptance"] = {
                ...
                "acceptance_type": "generic_create_file_controlled_business_source_change_acceptance",
                "new_dirty_freeze_path": str(freeze_path),
                "source_written_by_acceptance": False,
                "human_review_required": False,
                "machine_policy_gate": True,
    ...
    out_json = REPORT_DIR / f"controlled-change-generic-create-file-ledger-acceptance-{'apply' if args.apply else 'dry-run'}-{run_stamp}.json"
    out_md = SNAPSHOT_DIR / f"controlled-change-generic-create-file-ledger-acceptance-{'apply' if args.apply else 'dry-run'}-{run_stamp}.md"
    ...
    lines = []
    lines.append("# Learning V2 Controlled Change Generic Create File Ledger Acceptance")
    lines.append("")
    ...
    if acceptance_results:
        ...
        for item in acceptance_results:
            lines.append(f"- `{item.get('target_file')}` result=`{item.get('result')}` failures=`{item.get('failures')}`")
    ...
    print("controlled_change_generic_create_file_ledger_acceptance =", result)
