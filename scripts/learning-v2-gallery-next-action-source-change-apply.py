...
APPLY_ID = "learning-v2-gallery-next-action-source-change-apply-v0"
...
def main():
    ...
    ap.add_argument("--apply", action="store_true", help="actually write public/gallery.html after all gates pass")
    args = ap.parse_args()
    ...
    readiness_path, readiness = latest_report("gallery-next-action-gate-readiness-*.json")
    ...
    if rec_gate.get("human_review_required") is not False:
        failures.append("readiness_requires_human_review")

    if rec_gate.get("machine_policy_gate") is not True:
        failures.append("readiness_missing_machine_policy_gate")
    ...
    if args.apply and not failures:
        ...
        policy_obj = state.get("self_evolution_policy")
        if isinstance(policy_obj, dict):
            policy_obj["source_changes_allowed"] = True
            policy_obj["git_commit_allowed"] = False
            policy_obj["git_push_allowed"] = False
            policy_obj["deploy_allowed"] = False

        save_json(STATE, state)
        state_written = True

        backup_path = BACKUP_DIR / f"public-gallery-before-gallery-next-action-{stamp()}.html"
    ...
    payload = {
        ...
        "post_contains_gallery_next_action": "gallery-next-action" in post_text,
        "post_contains_gallery_next_action_title": "gallery-next-action-title" in post_text,
        "policy": {
    ...
    out_json = REPORT_DIR / f"gallery-next-action-source-change-apply-{suffix}-{stamp()}.json"
    out_md = REPORT_DIR / f"gallery-next-action-source-change-apply-{suffix}-{stamp()}.md"

    out_json.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lines = []
    lines.append("# Learning V2 Gallery Next Action Source Change Apply")
    lines.append("")
    ...
    print("gallery_next_action_source_change_apply =", payload["result"])
    ...
    print("post_contains_gallery_next_action =", str(payload["post_contains_gallery_next_action"]).lower())
    print("post_contains_gallery_next_action_title =", str(payload["post_contains_gallery_next_action_title"]).lower())
