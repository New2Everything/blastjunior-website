...
def write_report(state, apply):
    ...
    out = REPORT_DIR / f"dead-duplicate-proposal-finalizer-{suffix}-{stamp()}.md"

    review = state.get("last_dead_duplicate_review") or {}
    top = review.get("top_candidate") or {}

    lines = []
    lines.append("# Dead / Duplicate Proposal Finalizer")
    lines.append("")
    ...
    lines.append("")
    lines.append("This target family has completed an observe-only cycle.")
...
def main():
    ...
    print("dead_duplicate_proposal_finalizer =", "apply" if args.apply else "dry_run")
    ...
    if state.get("current_stage") != "dead_duplicate_proposal_ready":
        failures.append(f"current_stage_not_dead_duplicate_proposal_ready:{state.get('current_stage')}")
    ...
    if review.get("target_family") != TARGET_FAMILY:
        failures.append(f"missing_or_mismatched_last_dead_duplicate_review:{review.get('target_family')}")

    report_path = write_report(state, args.apply)

    if failures:
        print("dead_duplicate_proposal_finalizer_result = blocked")
    ...
    print("dead_duplicate_proposal_finalizer_result =", "track_complete" if args.apply else "would_track_complete")
    print("finalizer_report =", report_path)
    ...
    track = {
        ...
        "topic": "simplicity",
        "stage_before": "dead_duplicate_proposal_ready",
        "stage_after": "track_complete",
        "result": "track_complete",
        "reason": "observe-only dead/duplicate entry scan produced proposal-only evidence; source changes remain blocked",
        "proposal_review": state.get("last_dead_duplicate_review"),
