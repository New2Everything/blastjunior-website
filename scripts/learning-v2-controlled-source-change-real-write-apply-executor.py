...
SCRIPT_ID = "learning-v2-controlled-source-change-real-write-apply-executor-v0"
...
def main():
    ...
    if apply_request_auditor.get("audit_status") != "controlled_source_change_real_write_apply_request_ready_for_apply_executor_dry_run":
        hard_blocks.append("apply_request_auditor_not_ready_for_apply_executor")
    if apply_request_auditor.get("apply_executor_dry_run_allowed") is not True:
        hard_blocks.append("apply_executor_dry_run_not_allowed")
    ...
    if apply_request_auditor.get("deploy_allowed") is not False:
        hard_blocks.append("apply_request_auditor_deploy_allowed_too_early")
    if apply_request_auditor.get("hard_blocks"):
        hard_blocks.append("apply_request_auditor_has_hard_blocks")
    ...
    if route.get("github_repo") != EXPECTED_REPO:
        hard_blocks.append("request_route_github_repo_mismatch")
    if route.get("github_branch") != EXPECTED_BRANCH:
        hard_blocks.append("request_route_github_branch_mismatch")
    if route.get("deployment_platform") != EXPECTED_PLATFORM:
        hard_blocks.append("request_route_deployment_platform_mismatch")
    if route.get("deployment_trigger") != EXPECTED_TRIGGER:
        hard_blocks.append("request_route_deployment_trigger_mismatch")
    ...
    for item in apply_items:
        ...
        dry_run_execution.append({
            ...
            "would_write_if_apply_enabled": True,
    ...
    if hard_blocks:
        ...
        recommended_next_action = "fix_controlled_real_write_apply_executor_inputs"
        apply_executor_audit_allowed = False
    else:
        executor_status = "controlled_source_change_real_write_apply_executor_dry_run_ready_for_audit"
        recommended_next_action = "run_controlled_source_change_real_write_apply_executor_auditor_dry_run"
        apply_executor_audit_allowed = True
    ...
    payload = {
        ...
        "apply_executor_audit_allowed": apply_executor_audit_allowed,
    ...
    ts = stamp()
    json_path = REPORT_DIR / f"learning-v2-controlled-source-change-real-write-apply-executor-dry-run-{ts}.json"
    md_path = SNAPSHOT_DIR / f"learning-v2-controlled-source-change-real-write-apply-executor-dry-run-{ts}.md"

    json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    md = []
    md.append("# Learning V2 Controlled Source-Change Real-Write Apply Executor Dry Run")
    md.append("")
    ...
    md.append(f"- apply_executor_audit_allowed: `{str(apply_executor_audit_allowed).lower()}`")
    ...
    for x in dry_run_execution:
        ...
        md.append(f"- would_write_if_apply_enabled: `true`")
    ...
    print("controlled_source_change_real_write_apply_executor = ok")
    print("mode = dry_run")
    ...
    print("apply_executor_audit_allowed =", str(apply_executor_audit_allowed).lower())
