...
SCRIPT_ID = "learning-v2-controlled-source-change-real-write-apply-request-auditor-v0"
...
def main():
    ...
    if apply_request.get("request_status") != "controlled_source_change_real_write_apply_request_ready_for_audit":
        hard_blocks.append("apply_request_not_ready_for_audit")
    if apply_request.get("apply_request_audit_allowed") is not True:
        hard_blocks.append("apply_request_audit_not_allowed")
    ...
    if apply_request.get("deploy_allowed") is not False:
        hard_blocks.append("apply_request_deploy_allowed_too_early")
    if apply_request.get("hard_blocks"):
        hard_blocks.append("apply_request_report_has_hard_blocks")

    if executor_auditor.get("audit_status") != "controlled_source_change_real_write_executor_ready_for_apply_request_dry_run":
        hard_blocks.append("executor_auditor_not_ready")
    if executor_auditor.get("apply_request_dry_run_allowed") is not True:
        hard_blocks.append("executor_auditor_did_not_allow_apply_request")
    ...
    if packet.get("request_type") != "controlled_source_change_real_write_apply_request":
        hard_blocks.append("request_packet_type_invalid")
    if packet.get("requested_stage") != "controlled_source_change_real_write_apply":
    ...
    request_items = packet.get("apply_request_items") or []
    if not candidate_files:
        hard_blocks.append("missing_candidate_files")
    if not request_items:
        hard_blocks.append("missing_apply_request_items")

    required_next = set(packet.get("required_next_before_any_actual_write") or [])
    missing_next = sorted(REQUIRED_NEXT - required_next)
    hard_blocks.extend([f"missing_required_next:{x}" for x in missing_next])
    ...
    for item in request_items:
        ...
        if item.get("requires_apply_request_auditor") is not True:
            item_blocks.append("requires_apply_request_auditor_not_true")
        if item.get("requires_real_apply_executor_dry_run") is not True:
            item_blocks.append("requires_real_apply_executor_dry_run_not_true")
        ...
        if item.get("hard_blocks"):
            item_blocks.append("apply_request_item_has_hard_blocks")
    ...
    safety = apply_request.get("safety") or {}
    for k in [
        "business_source_written",
        "website_source_written",
        "git_commit",
    ...
    if hard_blocks:
        ...
        recommended_next_action = "fix_controlled_real_write_apply_request_before_apply_executor"
        apply_executor_dry_run_allowed = False
    else:
        audit_status = "controlled_source_change_real_write_apply_request_ready_for_apply_executor_dry_run"
        recommended_next_action = "run_controlled_source_change_real_write_apply_executor_dry_run"
        apply_executor_dry_run_allowed = True
    ...
    payload = {
        ...
        "apply_executor_dry_run_allowed": apply_executor_dry_run_allowed,
    ...
    ts = stamp()
    json_path = REPORT_DIR / f"learning-v2-controlled-source-change-real-write-apply-request-auditor-dry-run-{ts}.json"
    md_path = SNAPSHOT_DIR / f"learning-v2-controlled-source-change-real-write-apply-request-auditor-dry-run-{ts}.md"

    json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    md = []
    md.append("# Learning V2 Controlled Source-Change Real-Write Apply Request Auditor Dry Run")
    md.append("")
    ...
    md.append(f"- apply_executor_dry_run_allowed: `{str(apply_executor_dry_run_allowed).lower()}`")
    ...
    print("controlled_source_change_real_write_apply_request_auditor = ok")
    print("mode = dry_run")
    ...
    print("apply_executor_dry_run_allowed =", str(apply_executor_dry_run_allowed).lower())
