...
def main():
    ...
    recommended_changes = [
        {
            ...
            "change": "add accessibility-basics as executor-ready only for accessibility.navigation_semantics",
            "dry_run_first": True,
            "state_effect_if_apply_later": {
                "current_topic": "accessibility-basics",
                "current_stage": "accessibility_nav_probe",
        ...
        },
        {
            ...
            "change": "add route for accessibility_nav_probe to call accessibility-nav-semantics-probe only",
            "dry_run_first": True,
        ...
        {
            "file": "new resolver",
            "change": "later add accessibility_nav_probe_resolver to move probe evidence to review_ready",
            "dry_run_first": True,
            "source_write_risk": False,
        },
    ]
    ...
    report = {
        ...
        "design": "learning-v2-accessibility-family-design",
        ...
        "guardrails": {
            ...
            "probe_only_before_any_review_or_apply_executor": True,
        },
        "policy": {
            "state_written": False,
    ...
    out_json = REPORT_DIR / f"accessibility-family-design-{stamp()}.json"
    out_md = REPORT_DIR / f"accessibility-family-design-{stamp()}.md"

    out_json.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lines = []
    lines.append("# Learning V2 Accessibility Navigation Semantics Target-Family Design")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Why this target-family is safe")
    lines.append("")
    lines.append("This target-family observes accessibility semantics only. It checks nav landmarks, mobile menu button semantics, and visible link text. It must not create an apply-ready source-change path in learning_observe_only.")
    lines.append("")
    lines.append("## Required integration shape")
    lines.append("")
    lines.append("```text")
    lines.append("selector -> current_topic=accessibility-basics")
    lines.append("selector -> current_stage=accessibility_nav_probe")
    lines.append(f"selector -> current_target_family={TARGET_FAMILY}")
    lines.append("dispatch -> run accessibility_nav_semantics_probe only")
    lines.append("```")
    lines.append("")
    ...
    if review_items:
        for item in review_items[:8]:
            lines.append(f"- `{item.get('file')}:{item.get('line')}` `{item.get('kind')}` — {item.get('recommendation')}")
    else:
        lines.append("- none")
    lines.append("")
    ...
    print("accessibility_family_design =", result)
    ...
    print("recommended_selector_stage = accessibility_nav_probe")
