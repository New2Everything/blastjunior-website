...
SCRIPT_ID = "learning-v2-controlled-source-change-actual-write-executor-auditor-v0"
...
def main():
    ...
    if actual_write_executor.get("executor_status") != "controlled_source_change_actual_write_executor_dry_run_ready_for_audit":
        hard_blocks.append("actual_write_executor_not_ready_for_audit")
    if actual_write_executor.get("actual_write_executor_audit_allowed") is not True:
        hard_blocks.append("actual_write_executor_audit_not_allowed")
    if actual_write_executor.get("actual_source_write_allowed") is not False:
        hard_blocks.append("actual_write_executor_allows_actual_source_write_too_early")
    if actual_write_executor.get("actual_source_write_gate_opened") is not False:
        hard_blocks.append("actual_write_executor_opened_actual_source_write_gate")
    if actual_write_executor.get("actual_source_written") is not False:
        hard_blocks.append("actual_write_executor_wrote_actual_source")
    if actual_write_executor.get("deploy_allowed") is not False:
        hard_blocks.append("actual_write_executor_deploy_allowed_too_early")
    if actual_write_executor.get("hard_blocks"):
        hard_blocks.append("actual_write_executor_report_has_hard_blocks")
    ...
    if gate_opener_auditor.get("actual_write_executor_dry_run_allowed") is not True:
        hard_blocks.append("gate_opener_auditor_did_not_allow_actual_write_executor")
    ...
    route = actual_write_executor.get("deployment_route") or {}
    if route.get("github_repo") != EXPECTED_REPO:
        hard_blocks.append("actual_write_executor_route_github_repo_mismatch")
    if route.get("github_branch") != EXPECTED_BRANCH:
        hard_blocks.append("actual_write_executor_route_github_branch_mismatch")
    if route.get("deployment_platform") != EXPECTED_PLATFORM:
        hard_blocks.append("actual_write_executor_route_platform_mismatch")
    if route.get("deployment_trigger") != EXPECTED_TRIGGER:
        hard_blocks.append("actual_write_executor_route_trigger_mismatch")

    dry_run_execution = actual_write_executor.get("dry_run_execution") or []
    ...
    for item in dry_run_execution:
        ...
        if item.get("would_write_source_if_gate_opened_and_apply_enabled") is not True:
            item_blocks.append("would_write_source_if_gate_opened_and_apply_enabled_not_true")
        ...
        if item.get("hard_blocks"):
            item_blocks.append("dry_run_execution_item_has_hard_blocks")
    ...
    safety = actual_write_executor.get("safety") or {}
    ...
    if hard_blocks:
        ...
        recommended_next_action = "fix_actual_write_executor_before_post_write_validation"
        post_write_validation_dry_run_allowed = False
    else:
        audit_status = "controlled_source_change_actual_write_executor_ready_for_post_write_validation_dry_run"
        recommended_next_action = "run_controlled_source_change_post_write_validation_dry_run"
        post_write_validation_dry_run_allowed = True
    ...
    payload = {
        ...
        "post_write_validation_dry_run_allowed": post_write_validation_dry_run_allowed,
    ...
    ts = stamp()
    json_path = REPORT_DIR / f"learning-v2-controlled-source-change-actual-write-executor-auditor-dry-run-{ts}.json"
    md_path = SNAPSHOT_DIR / f"learning-v2-controlled-source-change-actual-write-executor-auditor-dry-run-{ts}.md"

    json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    md = []
    md.append("# Learning V2 Actual Write Executor Auditor Dry Run")
    md.append("")
    ...
    md.append(f"- post_write_validation_dry_run_allowed: `{str(post_write_validation_dry_run_allowed).lower()}`")
    ...
    print("controlled_source_change_actual_write_executor_auditor = ok")
    print("mode = dry_run")
    ...
    print("post_write_validation_dry_run_allowed =", str(post_write_validation_dry_run_allowed).lower())
