...
def write_report(state, apply):
    ...
    out = REPORT_DIR / f"accessibility-nav-proposal-finalizer-{suffix}-{stamp()}.md"

    review = state.get("last_accessibility_nav_review") or {}
    top = review.get("top_candidate") or {}

    lines = []
    lines.append("# Accessibility Nav Proposal Finalizer")
    lines.append("")
    ...
    lines.append(f"- top_candidate_type: `{top.get('proposal_type')}`")
    lines.append(f"- apply_allowed_now: `{str(top.get('apply_allowed_now')).lower()}`")
    lines.append("")
    lines.append("## Conclusion")
    lines.append("")
    lines.append("This target family has completed an observe-only accessibility semantics cycle.")
    lines.append("It produced proposal-only evidence, but no website source change is allowed in learning_observe_only.")
...
def main():
    ...
    print("accessibility_nav_proposal_finalizer =", "apply" if args.apply else "dry_run")
    ...
    if state.get("current_stage") != "accessibility_nav_proposal_ready":
        failures.append(f"current_stage_not_accessibility_nav_proposal_ready:{state.get('current_stage')}")
    ...
    if review.get("target_family") != TARGET_FAMILY:
        failures.append(f"missing_or_mismatched_last_accessibility_nav_review:{review.get('target_family')}")

    report_path = write_report(state, args.apply)

    if failures:
        print("accessibility_nav_proposal_finalizer_result = blocked")
    ...
    print("accessibility_nav_proposal_finalizer_result =", "track_complete" if args.apply else "would_track_complete")
    print("finalizer_report =", report_path)
    ...
    track = {
        ...
        "stage_before": "accessibility_nav_proposal_ready",
        "stage_after": "track_complete",
        "result": "track_complete",
        "reason": "observe-only accessibility navigation semantics scan produced proposal-only evidence; source changes remain blocked",
        "proposal_review": state.get("last_accessibility_nav_review"),
