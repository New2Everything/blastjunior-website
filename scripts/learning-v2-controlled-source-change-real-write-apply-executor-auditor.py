...
SCRIPT_ID = "learning-v2-controlled-source-change-real-write-apply-executor-auditor-v0"
...
def main():
    ...
    if apply_executor.get("executor_status") != "controlled_source_change_real_write_apply_executor_dry_run_ready_for_audit":
        hard_blocks.append("apply_executor_status_not_ready_for_audit")
    if apply_executor.get("apply_executor_audit_allowed") is not True:
        hard_blocks.append("apply_executor_audit_not_allowed")
    ...
    if apply_executor.get("deploy_allowed") is not False:
        hard_blocks.append("apply_executor_deploy_allowed_too_early")
    if apply_executor.get("source_change_gate_allowed") is not False:
        hard_blocks.append("apply_executor_source_change_gate_allowed_too_early")
    if apply_executor.get("source_change_gate_opened") is not False:
        hard_blocks.append("apply_executor_source_change_gate_opened")
    if apply_executor.get("hard_blocks"):
        hard_blocks.append("apply_executor_report_has_hard_blocks")
    ...
    if apply_request_auditor.get("actual_source_write_allowed") is not False:
        hard_blocks.append("apply_request_auditor_allows_actual_source_write_too_early")
    ...
    route = apply_executor.get("deployment_route") or {}
    if route.get("github_repo") != EXPECTED_REPO:
        hard_blocks.append("apply_executor_route_github_repo_mismatch")
    if route.get("github_branch") != EXPECTED_BRANCH:
        hard_blocks.append("apply_executor_route_github_branch_mismatch")
    if route.get("deployment_platform") != EXPECTED_PLATFORM:
        hard_blocks.append("apply_executor_route_platform_mismatch")
    if route.get("deployment_trigger") != EXPECTED_TRIGGER:
        hard_blocks.append("apply_executor_route_trigger_mismatch")

    dry_run_execution = apply_executor.get("dry_run_execution") or []
    ...
    for item in dry_run_execution:
        ...
        if item.get("would_write_if_apply_enabled") is not True:
            item_blocks.append("would_write_if_apply_enabled_not_true")
        ...
        if item.get("hard_blocks"):
            item_blocks.append("dry_run_item_has_hard_blocks")
    ...
    safety = apply_executor.get("safety") or {}
    ...
    if hard_blocks:
        ...
        recommended_next_action = "fix_controlled_real_write_apply_executor_before_actual_source_write_gate_request"
        actual_source_write_gate_request_allowed = False
    else:
        audit_status = "controlled_source_change_real_write_apply_executor_ready_for_actual_source_write_gate_request_dry_run"
        recommended_next_action = "run_controlled_source_change_actual_source_write_gate_request_dry_run"
        actual_source_write_gate_request_allowed = True
    ...
    payload = {
        ...
        "actual_source_write_gate_request_allowed": actual_source_write_gate_request_allowed,
    ...
    ts = stamp()
    json_path = REPORT_DIR / f"learning-v2-controlled-source-change-real-write-apply-executor-auditor-dry-run-{ts}.json"
    md_path = SNAPSHOT_DIR / f"learning-v2-controlled-source-change-real-write-apply-executor-auditor-dry-run-{ts}.md"

    json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    md = []
    md.append("# Learning V2 Controlled Source-Change Real-Write Apply Executor Auditor Dry Run")
    md.append("")
    ...
    md.append(f"- actual_source_write_gate_request_allowed: `{str(actual_source_write_gate_request_allowed).lower()}`")
    ...
    print("controlled_source_change_real_write_apply_executor_auditor = ok")
    print("mode = dry_run")
    ...
    print("actual_source_write_gate_request_allowed =", str(actual_source_write_gate_request_allowed).lower())
